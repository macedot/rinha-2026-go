...
# Apply PGO if profile exists (generated by profile.sh)
if [[ -f "$SCRIPT_DIR/cpu.pprof" ]]; then
    echo "Applying PGO from $SCRIPT_DIR/cpu.pprof" >&2
    cp "$SCRIPT_DIR/cpu.pprof" cmd/server/default.pgo
fi
